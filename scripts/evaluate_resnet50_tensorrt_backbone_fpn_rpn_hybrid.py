#!/usr/bin/env python3
"""Evaluate TensorRT backbone+FPN+RPN + PyTorch ROI heads hybrid."""
...
ENGINE_OUTPUT_NAMES = ("p2", "p3", "p4", "p5", "p6_pool", "proposals")
...
class TensorRTBackboneFPNRPNRunner:
    ...
    def _discover_tensors(self):
        ...
        if tuple(output_names) != ENGINE_OUTPUT_NAMES:
            raise RuntimeError(f"Expected TensorRT outputs {ENGINE_OUTPUT_NAMES}, got {tuple(output_names)}")
    ...
    def __call__(self, batch_tensor: torch.Tensor):
        if batch_tensor.device.type != "cuda":
            raise ValueError("TensorRT hybrid runner expects CUDA tensor input")
        ...
        if not ok:
            raise RuntimeError("TensorRT backbone+fpn+rpn execution failed.")
        self.stream.synchronize()
        features = OrderedDict(
            (model_name, self.output_tensors[engine_name])
            for model_name, engine_name in zip(MODEL_FEATURE_NAMES, ENGINE_OUTPUT_NAMES[:-1])
        )
        proposals = self.output_tensors["proposals"]
...
def evaluate_hybrid_model(model, runner, loader, device, progress_frequency=10, fixed_height=None, fixed_width=None):
    model.eval()
    predictions, targets = [], []
    ...
    if fixed_height is not None and fixed_width is not None:
        print(
            f"hybrid backbone-fpn-rpn evaluation started: target={total_images} images device={device} fixed_shape={fixed_height}x{fixed_width}",
            flush=True,
        )
    else:
        print(
            f"hybrid backbone-fpn-rpn evaluation started: target={total_images} images device={device} transform=model.transform(...)",
            flush=True,
        )
    ...
    for images, batch_targets in loader:
        ...
        features, proposals_tensor = runner(image_list.tensors)
        proposals = [proposals_tensor[index] for index in range(int(proposals_tensor.shape[0]))]
        ...
        if progress_frequency and (processed % progress_frequency < len(images) or processed >= total_images):
            print(f"hybrid backbone-fpn-rpn progress: {processed}/{total_images} images", flush=True)

    print("hybrid backbone-fpn-rpn inference completed; calculating detection metrics", flush=True)
    ...
    metrics["engine_input_shapes_seen"] = [list(shape) for shape in sorted(runner.shapes_seen)]
    if fixed_height is not None and fixed_width is not None:
        metrics["engine_shape"] = [int(fixed_height), int(fixed_width)]
    print("hybrid backbone-fpn-rpn evaluation completed", flush=True)
    return metrics
...
def main():
    ...
    if int(args.batch_size) != 1:
        raise ValueError("Hybrid TensorRT evaluation currently supports batch_size=1 only")
    config = load_config(args.config, require_dataset=True)
    device = choose_device(config.get("device", "auto"))
    if device.type != "cuda":
        raise RuntimeError("Hybrid TensorRT evaluation requires CUDA")
    loader = build_coco_loader(config, args.split, shuffle=False, limit=args.limit, batch_size=1)
    model = load_hybrid_model(config, args, device)
    runner = TensorRTBackboneFPNRPNRunner(args.engine)
    ...
    output = args.output or str(Path(config["output"]["directory"]) / "tensorrt_backbone_fpn_rpn_hybrid_eval.json")
